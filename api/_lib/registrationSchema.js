...
import { z } from "zod";
import { participationTypeOrder } from "./participationTypes.js";
...
export function createRegistrationSchema(validationMessages = defaultValidationMessages) {
  ...
  return z.object({
    name: z
      ...
      .pipe(z.string().min(2, messages.nameMin).regex(namePattern, messages.namePattern)),
    surname: z
      .string()
      .transform(sanitizeText)
      .pipe(z.string().min(2, messages.surnameMin).regex(namePattern, messages.surnamePattern)),
    email: z
      .string()
